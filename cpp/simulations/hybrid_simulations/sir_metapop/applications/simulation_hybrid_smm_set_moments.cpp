...
int main()
{
    ...
    const auto result_fct_smm =
        [](const mio::smm::SimulationSet<num_regions, mio::osir::InfectionState, max_order>& sim, double /*t*/) {
            return sim.get_mean();
        };
    const auto result_fct_moments = [](const mio::smm_moments::Simulation<num_regions, max_order>& sim, double /*t*/) {
