                                       ...
                                       double rel_switch_value)
{
    ...
    // Create simulations
    auto sim_smm     = mio::smm::Simulation<1, mio::osir::InfectionState>(smm_model, config.t0, config.dt);
    ...
    const auto condition = [rel_switch_value, &config](const mio::TimeSeries<double>& result_smm,
        ...
        if (smm_used) {
            ...
            if ((last_value[(int)mio::osir::InfectionState::Infected] >
                 rel_switch_value * config.total_populations[0]) ||
                (last_value[(int)mio::osir::InfectionState::Infected] < 1)) {
                return true;
            }
        }
        else {
