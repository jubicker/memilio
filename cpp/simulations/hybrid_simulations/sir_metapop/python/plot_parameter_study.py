...
def bin_labels(edges):
    return [
        fr"$\leq${format_bin_edge(edges[1])}" if e == -np.inf else
...
def plot_parameter_nan_grid(df, save_path, figsize, quantity):
    ...
    for use_custom_edges, suffix in [(False, "reg_bins"), (True, "bin_edges")]:
        ...
        cbar_label = "NaN count [#]" if use_custom_edges else "log(NaN count + 1)"
...
def plot_parameter_smape_grid(df, save_path, figsize, quantity):
    params = ["I_init_mean", "R_init_mean", "R0_mean", "std_I_init_norm"]
    n = len(params)
    ...
    bin_edges = {
        ...
        "R_init_mean": [-np.inf, 1000, 10000, 50000, np.inf],
        "R0_mean": [-np.inf, 1, 2, 3, np.inf],
        "std_I_init_norm": [-np.inf, 0, 0.1, 0.2, 0.5, 1, 2, np.inf]
    }

    smape = df[f"sMAPE_{quantity}"]
    cmap = LinearSegmentedColormap.from_list(
        "smape", ["white", colors["teal"], colors["dark teal"]])
    ...
    for use_custom_edges, suffix in [(False, "reg_bins"), (True, "bin_edges")]:
                ...
                if i == j:
                    ...
                    if use_custom_edges:
                        labels_x, labels_y, grid = custom_bins_2d(
                            param_x, param_y)
                        im = ax.pcolormesh(
                            grid, cmap=cmap, vmin=0, vmax=max_smape)
                        ax.set_xticks(np.arange(len(labels_x)) + 0.5)
                        ax.set_yticks(np.arange(len(labels_y)) + 0.5)
                        ax.set_xticklabels(
                            labels_x if j == n - 1 else [],
                        ...
                        im = ax.pcolormesh(
                            ...
                            cmap=cmap, vmin=0, vmax=max_smape)
                        if j != n - 1:
                            ax.set_xticklabels([])
                        if i != 0:
                            ax.set_yticklabels([])
        ...
        assert im is not None
        cbar_label = "Mean sMAPE"
        cbar = fig.colorbar(im, ax=axes, fraction=0.02, pad=0.02)
        cbar.set_label(cbar_label, labelpad=10)
        fig.savefig(save_path + f"smape_grid_{suffix}_{quantity}.png", dpi=dpi)
...
plot_parameter_nan_grid(df, base_folder, figsize=(3, 2.5), quantity="mean")
plot_parameter_nan_grid(df, base_folder, figsize=(3, 2.5), quantity="var")
plot_parameter_smape_grid(df, base_folder, figsize=(3, 2.5), quantity="mean")
plot_parameter_smape_grid(df, base_folder, figsize=(3, 2.5), quantity="var")
