...
def plot_mean_error(smm_dir, hybrid_dir, ode_dir,save_dir, switching_thresholds, colors_hybrid, color_ode, comp_index, num_regions, figsize):
    ...
    for r in range(num_regions):
        ...
        for i, st in enumerate(switching_thresholds):
            values = pd.read_csv(hybrid_dir + f"/{st}/means.csv")
...
def plot_variance_error(smm_dir, hybrid_dir, ode_dir,save_dir, switching_thresholds, colors_hybrid, color_ode, comp_index, num_regions, figsize):
    ...
    for r in range(num_regions):
        ...
        for i, st in enumerate(switching_thresholds):
            values = pd.read_csv(hybrid_dir + f"/{st}/moments.csv")
...
def plot_runtimes(smm_dir, hybrid_dir, ode_dir, save_dir, switching_thresholds, colors_hybrid, color_ode, color_smm, figsize):    
    ...
    for st in switching_thresholds:
        runtime = pd.read_csv(hybrid_dir + f"/{st}/runtimes.csv")
        mean_runtimes.append(runtime['Runtime'].mean())
        max_runtimes.append(runtime['Runtime'].max())
        runtime = pd.read_csv(hybrid_dir + f"/{st}/total_time.csv")
        total_runtime.append(runtime['Runtime'].iloc[0])
        labels.append(f"{st}")
...
def plot_mean_ts(dir, name_mean, compare_values, comp_index, num_regions, figsize, tmax, tmin, name2_mean="", dir_smm="", color_smm="", closure_order = "closure_order_3"):
    comp = list(compartment_colors.keys())[comp_index]
    
    for r in range(num_regions):
        for st in compare_values:
            fig, ax = plt.subplots(figsize=figsize)
            ...
            mean = pd.read_csv(dir + f"/{st}/{closure_order}/" + name_mean + ".csv")
            mean = mean[mean['Time'] <= tmax]
            mean = mean[mean['Time'] >= tmin]
            ax.plot(mean.Time, mean.iloc[:, 1 + comp_index + r * len(compartment_names)], color=compartment_colors[comp][0], label = r"$\mu$")
            
            if(name2_mean != ""):
                mean = pd.read_csv(dir + f"/{st}/{closure_order}/" + name2_mean + ".csv")
                mean = mean[mean['Time'] <= tmax]
                mean = mean[mean['Time'] >= tmin]
                ax.plot(mean.Time.iloc[1:], mean.iloc[1:, 1 + comp_index + r * len(compartment_names)], color=compartment_colors[comp][0])
                # Add line at swithcing tp
...
def plot_var_ts(dir, name_var, compare_values, comp_index, num_regions, figsize, name2_var="", dir_smm="", color_smm="", closure_order = "closure_order_3"):
    comp = list(compartment_colors.keys())[comp_index]
    for r in range(num_regions):
        moment_index = [0 for _ in range(len(compartment_names)*num_regions)]
        moment_index[comp_index + r * len(compartment_names)] = 2  # Variance moments: M200, M020, M002
        ...
        for st in compare_values:
            ...
            var = pd.read_csv(dir + f"/{st}/{closure_order}/" + name_var + ".csv")
            ax.plot(var.Time, var[col_name].iloc[:], color = compartment_colors[comp][0], label = r"$\sigma^2$")
            
            if(name2_var != ""):
                var = pd.read_csv(dir + f"/{st}/{closure_order}/" + name2_var + ".csv")
...
def plot_switching_times(hybrid_dir, save_dir, comparing_values, colors_hybrid, figsize):
    tps = []
    labels = []

    for i, st in enumerate(comparing_values):
        values = pd.read_csv(hybrid_dir + f"/{st}/means_smm.csv")
        tp =  values.iloc[-1, 0]
        tps.append(tp)
        labels.append(st)
    ...
    # Save summed error
    fig, ax = plt.subplots(figsize=(figsize[0], figsize[1]*0.8))
    x = range(len(comparing_values))
    width = 0.8 / num_regions
    bar_colors = colors_hybrid[0]
    ax.bar([i - width/2 for i in x], tps, width=width, color =  bar_colors)
    ax.set_xticks(x)
    ax.set_xticklabels(["" for _ in range(len(x))])
    ax.set_ylabel("Switching tp")
    fig.tight_layout()
    fig.savefig(save_dir + f"/switching_tps.png", dpi=dpi)
    plt.close()
    ...
    fig_leg.legend(handles, labels, loc='center')      
    fig_leg.tight_layout()
    fig_leg.savefig(save_dir + f"/legend_switch_tps.png", dpi=dpi, bbox_inches='tight', transparent=True)
    plt.close(fig_leg)
...
if __name__ == "__main__":
    ...
    # plot_mean_error(smm_dir, hybrid_dir, ode_dir, save_dir, compare_values, colors_hybrid, color_ode, comp_index, num_regions, figsize)
