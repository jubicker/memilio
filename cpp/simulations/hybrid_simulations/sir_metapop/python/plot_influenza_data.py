...
def plot_new_infections(sim_output_filepath, save_dir, real_data_file, start_date, tmax, region, pop_size, region_name, real_Altersgruppe=""):
    ...
    fig.savefig(save_dir +
                f"incidence_{region_name}_{real_Altersgruppe}.png")


def plot_infected_mean_std(sim_output_filepath, save_dir, start_date, region_name, tmax, plot_new_inf, col, col_var, pop_size=100000, real_data_file="", real_Altersgruppe=""):
    ...
    for t in range(0, len(df_mean)):
        sim_outputs["Date"].append(
            current_date + pd.to_timedelta(t/10., unit="D"))
    ...
    ax.fill_between(sim_outputs["Date"], np.maximum(sim_outputs["Mean"] - 2*sim_outputs["Std"], 0), sim_outputs["Mean"] + 2*sim_outputs["Std"],
    ...
    plt.grid()
    plt.tight_layout()
    fig.savefig(save_dir + f"mean_std_{region_name}_{real_Altersgruppe}.png")


def plot_infected_percentiles(sim_output_filepath, save_dir, start_date, p_lower, p_upper, region_name, tmax, col, real_data_file="", real_Altersgruppe=""):
    ...
    plt.tight_layout()
    fig.savefig(
        save_dir + f"percentiles_{region_name}_{real_Altersgruppe}.png")


# "V:/bick_ju/TemporalHybrid/Moments/config_influenza_germany/truncation/closure_order_3/0.000000/"#"V:/bick_ju/TemporalHybrid/SMM/"
sim_output_dir = "/Users/julia/sim_outputs/output/SMM/"
grippe_web_data = "/Users/julia/repos/grippeweb_data/ILI_df_age_groups.csv"
start_date = pd.to_datetime("2016-08-01")
# "H:/Documents/TemporalHybridModel/Moments/"#"H:/Documents/TemporalHybridModel/SMM/"
save_dir = "/Users/julia/sim_outputs/output/SMM/"
config = "config_influenza_agegroups"
...
col_vars = ["M020000000000000", "M000020000000000",
            "M000000020000000", "M000000000020000", "M000000000000020"]
Altergruppen = ["0-4", "5-14", "15-34", "35-59", "60"]

os.makedirs(save_dir, exist_ok=True)

for i, Altergruppe in enumerate(Altergruppen):
    col = cols[i]
    col_var = col_vars[i]
    plot_infected_mean_std(sim_output_dir,
                           save_dir, start_date, region_name, tmax, col=col, col_var=col_var, plot_new_inf=True, real_data_file=grippe_web_data, real_Altersgruppe=Altergruppe)

    plot_infected_percentiles(sim_output_dir,
                              save_dir, start_date, "p05", "p95", region_name, tmax, col=col, real_data_file=grippe_web_data, real_Altersgruppe=Altergruppe)

    plot_new_infections(sim_output_dir, save_dir,
                        grippe_web_data, start_date, tmax, region, pop_size, region_name, real_Altersgruppe=Altergruppe)
