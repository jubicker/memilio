...
/**
* @brief Initializes an SMM with the given config.
...
{
    size_t num_seasons = config.season_peaks.size();
    assert(config.num_regions == NumRegions);
    mio::smm::Model<ScalarType, NumRegions, mio::osir::InfectionState> model(num_seasons);
    ...
    for (size_t r = 0; r < config.num_regions; ++r) {
        ...
        adoption_rates.push_back({mio::osir::InfectionState::Infected,
                                  ...
                                  config.gamma,
                                  {}});
        // Immunity loss rate is also the same for all regions - is only initilized if > 0
        if (config.nu > 0) {
    ...
    // Initialize transition rates only if there is exchange between regions
    if (config.transition_rates.size() > 0) {
        model.parameters.template get<mio::smm::TransitionRates<ScalarType, mio::osir::InfectionState>>() =
            config.transition_rates;
    }

    // Initialize seasonality parameters if there are any
